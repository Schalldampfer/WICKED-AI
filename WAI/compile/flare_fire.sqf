...
_interval = 150 + random 60; //Interval of firing in seconds

//_this = group;
_grp = _this;
_unit = leader _this;
if (WAI_DebugMode) then {diag_log format["WAI: Preparing Flare of %1",name _unit];};
...
while {alive _unit} do {
	if (sunOrMoon != 1) then {
		...
		_targ = _unit;
		{
			if (alive _x && ((_grp knowsAbout _x) > 1.5 || !terrainIntersectASL [aimPos _unit,aimPos _x]) && (_unit distance _x) < 1300) then {_targ = _x};
		} forEach playableUnits;
		//if found
		if (_targ != _unit) then {
			if (WAI_DebugMode) then {diag_log format["WAI: Firing Flare on %1",name _targ];};
			//aim target pos
			_pos = getPos _targ;
